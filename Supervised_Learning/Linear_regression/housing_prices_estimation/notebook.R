...
corrplot(M, method = "color",
         ...
         is.corr=T, #Es correlación? no sé pa que es éste
...
par( mfcol = c( 1, 2))
y<-my_data$MEDV
hist(NULL,prob=T,main="Histograma de Frecuencias Relativas, variable MEDV",ylab=
       "Densidad", xlab="MEDV")
lines(density(y),col="blue")
qqnorm(NULL,main="Normal Q-Q Plot, variable MEDV",ylab="Cuantiles de Muestra",
       xlab="Cuantiles Teóricos")
qqline(NULL,col="blue")
lillie.test(NULL)
...
my_data<-my_data[c(-NULL,-NULL,-NULL,-NULL,-NULL,-NULL,-NULL,-NULL,-NULL,-NULL),]
...
wor<-exp(predict.lm(NULL,NULL,interval="confidence"))

pred1<-data.frame(x4=1,x5=0.508,x6=5.575,x8=4.19,
                  x11=16.3, x12=321.9, x13=2.98)
wr<-exp(predict.lm(NULL,NULL,interval="confidence"))
...
my_pred<-as.data.frame(exp(predict.lm(NULL,newdata=NULL,interval="confidence",level = 0.99)))

result_conf<-ifelse( NULL< my_data$MEDV & my_data$MEDV < NULL,1,0)
